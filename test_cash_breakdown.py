...
class CashBreakdownNormalizationTests(unittest.TestCase):
    def test_uses_transfer_alias_and_preserves_total(self):
        breakdown = _normalize_declared_breakdown(
            {'efectivo': 1200, 'pos': 800, 'qr': 350, 'trans': 500, 'otros': 100}
        )

        self.assertEqual(
            breakdown,
            {'efectivo': 1200, 'pos': 800, 'qr': 350, 'transferencia': 500, 'otros': 100},
        )
        self.assertEqual(_breakdown_total(breakdown), 2950)
    ...
    def test_falls_back_to_cash_when_breakdown_is_missing(self):
        ...
        self.assertEqual(
            breakdown,
            {'efectivo': 1750, 'pos': 0, 'qr': 0, 'transferencia': 0, 'otros': 0},
        )
    ...
    def test_keeps_legacy_pos_without_forcing_qr_split(self):
        breakdown = _normalize_declared_breakdown({'pos': 900}, fallback_amount=900)

        self.assertEqual(
            breakdown,
            {'efectivo': 0, 'pos': 900, 'qr': 0, 'transferencia': 0, 'otros': 0},
        )
